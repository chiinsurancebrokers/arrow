...
def database_url() -> str:
    """Production uses Railway's DATABASE_URL. SQLite is a local/test fallback."""
...
def _ensure_schema_and_roster() -> None:
    eng = engine()
    metadata.create_all(eng)
    with eng.begin() as conn:
        count = conn.execute(select(func.count()).select_from(employees_table)).scalar_one()
        if count:
            return
        ...
        for employee in baseline.get("employees", []):
            ...
            if not name:
                continue
            rows.append({"name": name, "email": str(employee.get("email", "")).strip()})
        if rows:
            conn.execute(insert(employees_table), rows)
